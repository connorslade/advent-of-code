...
fn part_a(input: &str) -> Answer {
    ...
    let mut sum = 0;
    for i in 0..problem.updates.len() {
        if problem.is_valid(i) {
            let update = &problem.updates[i];
...
fn part_b(input: &str) -> Answer {
    ...
    for i in 0..problem.updates.len() {
        if !problem.is_valid(i) {
            let update = problem.sort_pages(i);
            sum += update[update.len() / 2];
        }
    }
...
impl Problem {
    ...
    fn is_valid(&self, idx: usize) -> bool {
        ...
        line.is_sorted_by(|a, b| self.rule_map[b].contains(a))
    }

    fn sort_pages(&self, idx: usize) -> Vec<u32> {
        let mut line = self.updates[idx].clone();
        line.sort_by(|a, b| self.rule_map[b].contains(a).cmp(&true));
        line
    }
}
